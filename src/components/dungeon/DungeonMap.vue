...
function getRoom(x: number, y: number): Room | undefined {
  return props.rooms.find((room) => room.x === x && room.y === y);
}
</script>

<template>
  <div class="overflow-x-hidden border-4 border-gb-darker bg-gb-dark p-3">
    <div class="grid grid-cols-4 gap-2">
      <template v-for="y in 4" :key="`row-${y}`">
        <div
          v-for="x in 4"
          :key="`cell-${x}-${y}`"
          class="aspect-square cursor-pointer border-4 border-gb-darker bg-gb-light text-xl sm:text-2xl flex items-center justify-center relative active:scale-95 transition-all duration-100"
    ...
    <div class="mt-3 flex flex-wrap justify-center gap-3 text-xs text-gb-bg">
      <span>🔒 未解锁</span>
      <span>⚔️ 学习中</span>
      <span>✓ 已通关</span>
    </div>
